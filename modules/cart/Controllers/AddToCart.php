...
class AddToCart
{
    ...
    public function __invoke(Request $request, Response $response): Response {
        ...
        $quantity = (float) (!empty($post['quantity']) ? $post['quantity'] : 0);
        $custcolor = (!empty($post['color']) ? $post['color'] : '{"color":"#FFFFFF"}');

		if (($quantity >= 0) && ($v4warehouses_id > 0)) {
			$cart_data = new CartModel($this->db, $this->container, $this->session, $this->log);
			$cart_result = $cart_data->AddItem($v4warehouses_id, $quantity, $custcolor, $language);
		}
		else {
			$cart_result = array(
                'success' => 0,
