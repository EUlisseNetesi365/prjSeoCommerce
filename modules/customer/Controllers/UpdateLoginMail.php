...
class UpdateLoginMail
{
    ...
    public function __construct(App $app, ContainerInterface $container, SessionInterface $session, DBDefault $db, Twig $twig, LoggerInterface $logger)
    {
        ...
        $this->twig = $twig;
        $this->log = $logger;
        $this->mailupdate = new Customer($db, $container, $session, $this->log);
    ...
    public function __invoke(Request $request, Response $response): Response {
        ...
        $emailId = (string) (!empty($post['id']) ? $post['id'] : 0);
        $result = $this->mailupdate->updateMailLogin($customer_id, $emailId);
